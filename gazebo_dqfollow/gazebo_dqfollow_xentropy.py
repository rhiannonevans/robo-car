...
def iterate_batches(env, net, batch_size):
    ...
    # consists of a 10 element tuple with relative line position information:
    #   * [0,0,0,0,0,0,0,0,0] with one 0 being 1, depending on the position of the line
    obs = env.reset()
    ...
    while True:
        ...
        print("Action probabilities: " + str(act_probs))
...
if __name__ == '__main__':
    ...
    obs_size = env.observation_space.n  # Set observation space size
    n_actions = env.action_space.n  # Set action space size
    ...
    writer = SummaryWriter(comment="-deepQCar")
    ...
    for iter_no, batch in enumerate(iterate_batches(env, net, BATCH_SIZE)):
        ...
        # episodes
        '''
            @ obs_v is the input ('x') tensor, then we send each through the network which predicts what the action SHOULD be
        '''
